...
public class DeadGrassFeature extends Feature<NoneFeatureConfiguration> {

    public DeadGrassFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
            ...
            if (ground != null) {
                ...
                if (level.isEmptyBlock(placePos)) {
                    BlockState state = random.nextBoolean()
                        ? ModBlocks.DEAD_GRASS.get().defaultBlockState()
                        : ModBlocks.SHORT_DEAD_GRASS.get().defaultBlockState();
